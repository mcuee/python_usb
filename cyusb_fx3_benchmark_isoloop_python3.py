...
def read_loop():
    ...
    trans = 0
    while True:
        data = dev.read(epin, chunkr, 1000)
        #datareadback = ''.join([chr(x) for x in data])
        #print(len(datareadback))
        #assert datareadback == dat
        trans = trans + len(data)
        if time.time() > start + 1:
            bps = trans/(time.time() - start)
            print ("Read: transferred %d kB/s" % (bps/(1024)) )
            trans = 0
            start = time.time()
